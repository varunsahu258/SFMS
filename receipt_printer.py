...
def print_receipt(conn, receipt_no, reprint=False):
    """Generate a BIG or SMALL receipt PDF, optionally tracking a duplicate reprint."""
    ...
    pdf_path = str(Path(RECEIPTS_DIR) / f"{receipt_no}.pdf")
    temp_file = tempfile.NamedTemporaryFile(prefix="sfms_qr_", suffix=".png", delete=False)
    temp_file.close()
    try:
        qrcode.make(qr_payload).save(temp_file.name)
        qr_reader = ImageReader(temp_file.name)
    ...
    if reprint:
        ...
        conn.execute(
            ...
            SET reprint_count = COALESCE(reprint_count, 0) + 1,
                last_reprint_at = ?, last_reprint_by = ?
            WHERE receipt_no = ?
            """,
            (timestamp, user_id, receipt_no),
        )
        log_action(
            conn,
            user_id,
