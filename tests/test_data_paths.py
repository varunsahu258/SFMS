...
def test_platform_paths(monkeypatch, tmp_path):
    ...
    assert data_paths.get_app_config_dir("Windows") == (tmp_path / "localappdata" / "SFMS").resolve()
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg"))
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path / "config"))
    assert data_paths.get_app_data_dir("Linux") == (tmp_path / "xdg" / "SFMS").resolve()
    assert data_paths.get_app_config_dir("Linux") == (tmp_path / "config" / "SFMS").resolve()
...
def test_configured_paths_are_inside_base_dir():
    ...
    assert Path(config.INTEGRITY_KEY_PATH).resolve().is_relative_to(config.CONFIG_DIR.resolve())
    assert not Path(config.INTEGRITY_KEY_PATH).resolve().is_relative_to(config.BASE_DIR.resolve())
